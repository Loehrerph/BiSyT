...
%% load table and extract needed log free energy 

% lme = readtable('GCM_F.xlsx');
% % lme (:,1:4) = [];
% % lme (:,4:end) = [];
% lme (:,1:8) = [];
% lme (1,:) = [];
% lme = table2cell(lme);
% lme = str2double(lme);
% save ('Contrast.mat', 'lme');

%% load preprocessed log free energy

 load ('contrast.mat');

 %% use only if you want to calculate probabilities for groups separately
